...
router.post('/requests', async (req, res) => {
  ...
  try {
    ...
    try {
        ...
        // A. Get Hospital Name
        const hospRes = await pool.query('SELECT name FROM users WHERE id = $1', [req.user.id]);
        ...
        if (tokens.length > 0) {
            ...
            console.warn("[Notify] WARNING: No donors found. Notification skipped.");
        }
    ...
    console.log(`=== [POST /requests] Completed in ${Date.now() - start}ms ===\n`);
